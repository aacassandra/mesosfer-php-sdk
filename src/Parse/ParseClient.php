...
final class ParseClient
{
    ...
    public static function _request(
        ...
        if ($appRequest) {
            ...
            $httpClient->addRequestHeader(config('mesosfer.' . $env . '.headerAccountKey'), self::$accountKey);
        } else {
            self::assertParseInitialized();

            // add appId & client version
            $httpClient->addRequestHeader(config('mesosfer.' . $env . '.headerAppID'), self::$applicationId);
            $httpClient->addRequestHeader(config('mesosfer.' . $env . '.headerClientVersion'), self::VERSION_STRING);


            if ($sessionToken) {
                // add our current session token
                $httpClient->addRequestHeader(config('mesosfer.' . $env . '.headerSessionToken'), $sessionToken);
            }

            if ($useMasterKey) {
                // pass master key
                $httpClient->addRequestHeader(config('mesosfer.' . $env . '.headerMasterKey'), self::$masterKey);
            } elseif (isset(self::$restKey)) {
                // pass REST key
                $httpClient->addRequestHeader(config('mesosfer.' . $env . '.headerRestKey'), self::$restKey);
            }

            if (self::$forceRevocableSession) {
                // indicate we are using revocable sessions
                $httpClient->addRequestHeader(config('mesosfer.' . $env . '.headerRevocableSession'), '1');
            }
        }
